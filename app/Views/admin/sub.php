    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix py-3">
                <h4 class="page-title pull-left">Kelola subkriteria</h4>
...
<div class="main-content-inner">
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    ...
                    <h4 class="header-title">Subkriteria <button id="tambah" class="ml-1 btn btn-primary btn-xs">tambah
    ...
    $sub = $ahp->sub->where(['kriteria_id' => $k['kriteria_id']])->findAll();?>
...
<script>
function edit(id) {
    fetch('/ahp/sub/get/' + id)
        .then(e => e.json())
        .then(e => {
            Swal.fire({
                title: 'ubah data',
                html: `
            <form action="/ahp/sub/edit/${id}" method="post">
                        <div class="input-group">
                            <label class="text-left col-12 p-0 mx-0 my-2" for="nama">Nama</label>
                            <input value="${e.sub_nama}" class="form-control" name="nama" type="text" id="nama">
...
function hapus(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = "/ahp/sub/hapus/" + id;
...
window.onload = function() {
    ...
    $("#tambah").click(e => {
        Swal.fire({
            title: 'tambah data',
            html: `
            <form action="<?=route_to('add_sub');?>" method="post">
                        <div class="input-group">
                            <label class="text-left col-12 p-0 mx-0 my-2" for="kriteria">Kriteria</label>
                            <select class="form-control" name="kriteria" type="text" id="kriteria">
                            <?php foreach ($kriteria as $k): ?>
                                 <option value="<?=$k['kriteria_id'];?>"><?=$k['kriteria_nama'];?></option>
                            <?php endforeach?>
                            </select>
                            <div class="text-danger"></div>
                        </div>
